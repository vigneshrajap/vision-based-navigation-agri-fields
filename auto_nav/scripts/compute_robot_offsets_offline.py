...
def interpolate_position_to_new_time(pos_time, pos_x, pos_y, new_time):  
    f_interp_x = interp1d(x=pos_time, y = pos_x,fill_value = 'extrapolate')
    f_interp_y = interp1d(x=pos_time, y = pos_y,fill_value = 'extrapolate')
...
def compute_offset(pos_ind, r_pos_x, r_pos_y, gt_x, gt_y, debug = False):
    ...
    r_point,r_vec = line_to_next_point(pos_ind,r_pos_x, r_pos_y,step=5)
